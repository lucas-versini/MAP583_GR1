...
def train(train_loader, model, criterion, optimizer, epoch, device):
    ...
    with tqdm(train_loader) as _tqdm:
        for x, y in _tqdm:
            ...
            # calc accuracy
            predicted = outputs.int().float()
            correct_num = predicted.squeeze().eq(y).sum().item()
...
def validate(validate_loader, model, criterion, epoch, device):
        ...
        with tqdm(validate_loader) as _tqdm:
            for i, (x, y) in enumerate(_tqdm):
                ...
                if criterion is not None:
                    ...
                    # calc accuracy
                    predicted = outputs.squeeze().int().float()
    ...
    return loss_monitor.avg, accuracy_monitor.avg, mae, ave_preds, gt
...
def main():
    ...
    for epoch in range(start_epoch, cfg.TRAIN.EPOCHS):
        ...
        # validate
        val_loss, val_acc, val_mae, ave_preds, gt = validate(val_loader, model, criterion, epoch, device)
        ...
        if val_mae < best_val_mae:
            ...
            torch.save(
                ...
                str(checkpoint_dir.joinpath(f"id_{id_}_" + "epoch{:03d}_{:.5f}_{:.4f}.pth".format(epoch, val_loss, val_mae)))
            )
            list_models_saved.append(f"id_{id_}_" + "epoch{:03d}_{:.5f}_{:.4f}.pth".format(epoch, val_loss, val_mae))
