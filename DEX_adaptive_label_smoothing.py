...
def smooth_labels(y_hard, n_classes, smoothing=0.1):
    """
    Convert hard labels to soft labels.
    :param y_hard: tensor of hard labels, shape [n_batch]
    :param n_classes: total number of classes
    :param smoothing: smoothing factor (default 0.1)
    :return: tensor of soft labels, shape [n_batch, n_classes]
    """
    assert 0 <= smoothing < 1
    # Create an empty tensor for soft labels with smoothing applied
    y_soft = torch.full(size=(y_hard.size(0), n_classes), fill_value=smoothing / (n_classes - 1), device=y_hard.device)
    # Subtract smoothing from the true class index
    y_soft.scatter_(1, y_hard.unsqueeze(1), 1. - smoothing + smoothing / (n_classes - 1))
    return y_soft




def train(train_loader, model, criterion, optimizer, epoch, device, epochs = 80):
    model.train()
    ...
    with tqdm(train_loader) as _tqdm:
        for x, y_hard in _tqdm:
            ...
            y = smooth_labels(y_hard, 101, 0.15 * (1 - epoch / epochs))
...
def main():
    ...
    for epoch in range(start_epoch, cfg.TRAIN.EPOCHS):
        ...
        # train
        train_loss, train_acc = train(train_loader, model, criterion, optimizer, epoch, device, cfg.TRAIN.EPOCHS)
